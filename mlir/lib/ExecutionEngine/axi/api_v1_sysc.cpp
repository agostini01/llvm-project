...
void dma::dma_init(uint64_t _dma_address, uint64_t _dma_input_address,
  ...
  dma_input_address =
      (uint64_t *)malloc(_dma_input_buffer_size * sizeof(uint64_t));
  dma_output_address =
      (uint64_t *)malloc(_dma_output_buffer_size * sizeof(uint64_t));

  // Initialize with zeros
  for (uint64_t i = 0; i < _dma_input_buffer_size; i++) {
    *(dma_input_address + i) = 0;
  }

  for (uint64_t i = 0; i < _dma_output_buffer_size; i++) {
  ...
  dm.DMA_input_buffer = (int64_t *)dma_input_address;
  dm.DMA_output_buffer = (int64_t *)dma_output_address;
