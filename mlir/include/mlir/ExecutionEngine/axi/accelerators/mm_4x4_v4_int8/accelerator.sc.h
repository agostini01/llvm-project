...
SC_MODULE(ACCNAME) {
  sc_in<bool> clock;
  sc_in<bool> reset;
  acc_t A_buffer[256][16];
  acc_t B_buffer[256][16];
  acc_t C_buffer[256][16];
  ...
  void Compute(acc_t[PE_M][PE_K], acc_t[PE_K][PE_N],
               acc_t[PE_M][PE_N]);

  void LoadA(acc_t[PE_M][PE_K], su10, su10, su10);

  void LoadB(acc_t[PE_K][PE_N], su10, su10, su10);

  void Store(acc_t[PE_M][PE_N], su10, su10, su10);
  ...
  while (1) {
    ...
    if (packet.read_A) {
      ...
      for (int i = 0; i < read_length; i++) {
        for (int j = 0; j < 16; j++) {
          A_buffer[i][j] = din1.read().data.range(7, 0);
          read_A_len++;
          DWAIT();
        }
      }
    ...
    if (packet.read_B) {
      ...
      for (int i = 0; i < read_length; i++) {
        for (int j = 0; j < 16; j++) {
          B_buffer[i][j] = din1.read().data.range(7, 0);
          read_B_len++;
          DWAIT();
        }
      }
...
void ACCNAME::LoadA(acc_t A[PE_M][PE_K], su10 M, su10 K, su10 in_stride) {
...
void ACCNAME::LoadB(acc_t B[PE_K][PE_N], su10 K, su10 N, su10 in_stride) {
...
void ACCNAME::Compute(acc_t A[PE_M][PE_K], acc_t B[PE_K][PE_N],
                      acc_t C[PE_M][PE_N]) {
...
void ACCNAME::Store(acc_t C[PE_M][PE_N], su10 M, su10 N, su10 out_stride) {
