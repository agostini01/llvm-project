...
void dma::dma_init(unsigned int _dma_address, unsigned int _dma_input_address,
  ...
  void *dma_mm = mmap(NULL, 65536, PROT_READ | PROT_WRITE, MAP_SHARED, dh,
...
int dma::dma_copy_to_inbuffer(unsigned int *src_address, int data_length,
                              int offset) {
  assert(("data copy will overflow input buffer",
          offset + data_length <= dma_input_buffer_size));
...
int dma::dma_copy_from_outbuffer(unsigned int *dst_address, int data_length,
                                 int offset) {
  assert(("tries to access data outwith the output buffer",
          offset + data_length <= dma_output_buffer_size));
  std::memcpy(&dma_input_address + offset, &dst_address, sizeof(data_length));
}
//==============================================================================

int dma::dma_start_send(int length, int offset) {
  assert(("trying to send data outside the input buffer",
...
int dma::dma_start_recv(int length, int offset) {
  assert(("trying receive data outside the output buffer",
...
unsigned int dma::dma_set(unsigned int *dma_address, int offset,
                          unsigned int value) {
